...
export default async function BillDetailPage({
  ...
  // Check for R&D allocations via journal lines linked to bill line accounts
  const rdJournalLines = await prisma.journalLine.findMany({
    where: {
      journalEntry: {
        sourceType: "Bill",
        sourceId: bill.id,
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span
              className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${
                statusBadge[bill.status] || "bg-gray-100 text-gray-700"
              }`}
            >
              {bill.status}
            </span>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Bill from{" "}
            <Link href={`/contacts/${bill.contact.id}`} className="text-indigo-600 hover:text-indigo-800">
              {bill.contact.name}
            </Link>
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link
            href={`/bills/${bill.id}/email-preview`}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {bill.notes && (
          <div className="mt-6 border-t border-slate-100 pt-4">
            ...
            <dd className="mt-1 text-sm text-slate-700">{bill.notes}</dd>
          </div>
        )}
      </div>
      ...
      {/* R&D Allocation Section */}
      {rdJournalLines.length > 0 && (
        <div className="rounded-xl border border-violet-200 bg-violet-50 shadow-sm">
          <div className="border-b border-violet-200 px-6 py-4">
            <h2 className="text-sm font-semibold text-violet-900">R&D Allocation</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-violet-200">
              <thead className="bg-violet-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-violet-600">
                    R&D Project
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-violet-600">
                    Account
                  ...
                  <th className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-violet-600">
                    Debit
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-violet-600">
                    Credit
              ...
              <tbody className="divide-y divide-violet-100 bg-white">
                {rdJournalLines.map((jl) => (
                  <tr key={jl.id}>
                    <td className="px-6 py-3 text-sm font-medium text-violet-700">
                      {jl.rdProject ? (
                        <Link href={`/rd/projects/${jl.rdProject.id}`} className="hover:text-violet-900">
                          {jl.rdProject.name}
                        </Link>
                      ) : (
                        "\u2014"
                    ...
                    <td className="px-6 py-3 text-sm text-slate-600">
                      {jl.account.code} - {jl.account.name}
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600">
                      {jl.description || "\u2014"}
                    </td>
