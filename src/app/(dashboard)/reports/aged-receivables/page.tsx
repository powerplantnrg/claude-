...
export default async function AgedReceivablesPage() {
  ...
  for (const inv of invoices) {
    const daysOverdue = calculateDaysOverdue(inv.dueDate, today)
    ...
    if (daysOverdue <= 0) {
      entry.current += inv.total
    } else if (daysOverdue <= 30) {
      entry.days1to30 += inv.total
    } else if (daysOverdue <= 60) {
      entry.days31to60 += inv.total
    } else if (daysOverdue <= 90) {
      entry.days61to90 += inv.total
    } else {
      entry.days90plus += inv.total
    }
  }
  ...
  for (const inv of invoices) {
    const daysOverdue = calculateDaysOverdue(inv.dueDate, today)
    totalWeightedDays += Math.max(0, daysOverdue) * inv.total
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Overdue Contacts</p>
